...
static inline int f2fs_check_avail_size
	(struct dentry *dentry, size_t size, int dir)
{
	struct super_block *sb = dentry->d_sb;
	struct f2fs_sb_info *sbi = F2FS_SB(sb);
	u64 avail;

	if ((!sbi->mount_opt.reserved_mb) ||
		(!sbi->mount_opt.reserved_fsuid))
		return 1;

	avail = (u64)sbi->user_block_count * sbi->blocksize -
			(u64)valid_user_blocks(sbi) * sbi->blocksize;
...
out:
	pr_err("f2fs low memory, avail size=%lldMB, fsuid=%d, current_comm=%s\n",
		avail/1024/1024,
